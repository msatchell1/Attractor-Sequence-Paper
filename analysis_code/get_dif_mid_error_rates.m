function [dif_mid_error_rates,not_dif_mid_error_rates] = get_dif_mid_error_rates(datadir)
...
% 2-choice error rates among sequences where the 3rd stimulus in the
% sequence is or is not the "same" or "opposite" the correct choice. e.g.
% in the sequence "LLRLRL" the 3rd stimulus (R="right") is opposite the
% correct choice ("left")
%
% Inputs:
...
%
% Outputs:
% 1. dif_mid_error_rates: (nv1 x nv2 x nnets) matrix where each element
%                           (i,j,k) gives the mean choice error rate among
%                           sequences where the 3rd stimulus is opposite
%                           the correct choice for the k'th network tested
%                           with the i'th value of parameter 1 and the j'th
%                           value of parameter 2
% 2. not_dif_mid_error_rates: The same as dif_first_error_rates but for
%                               sequences where the 3rd stimulus equals the
%                               correct choice
...
for i=1:nv1
    for j=1:nv2
        for k=1:nnets
            ...
            dif_mid = (not_even_seqs(:,3)~=not_even_target');
            errors = (not_even_y ~= not_even_target);
            dif_mid_errors = errors(dif_mid);
            ndif_mid_errors(i,j,k) = sum(dif_mid_errors);
            n_not_dif_mid_errors(i,j,k) = sum(errors(~dif_mid));
            dif_mid_error_rates(i,j,k) = ndif_mid_errors(i,j,k)/sum(dif_mid);
            not_dif_mid_error_rates(i,j,k) = n_not_dif_mid_errors(i,j,k)/sum(~dif_mid);
        end
    end
end
end
