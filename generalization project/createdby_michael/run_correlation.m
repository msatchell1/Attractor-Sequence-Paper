...
p = make_params_genr("stim_seed",5, "num_char",2, "num_type",2,...
        "net_seed",17, "Ne",100, "stim_dur",stim_dur, "mean_stim_amp",stim_amp,...
...
param_array = 0:0.01:1; % remember matlab uses start:step:stop
param_name = "Stimulus Type Fraction";
...
nostim_on_fin_SEMs = zeros(length(param_array),1);
stim_on_fin_avgs = zeros(length(param_array),1);
stim_off_fin_avgs = zeros(length(param_array),1);
stim_off_fin_SEMs = zeros(length(param_array),1);
...
for k = 1:length(param_array)
    ...
    stim_off_fin_temps = zeros(num_sims,1);
    nostim_on_fin_temps = zeros(num_sims,1);
    ...
    for i = 1:num_sims
        ...
        stim_off_fin_temps(i) = sum(stim_offu_fin);
        nostim_on_fin_temps(i) = sum(nostim_onu_fin);
...
%         cc_temps(i) = corr_coef;
    ...
    stim_off_fin_avgs(k) = mean(stim_off_fin_temps);
    stim_off_fin_SEMs(k) = std(stim_off_fin_temps)/sqrt(length(stim_off_fin_temps));
    nostim_on_fin_avgs(k) = mean(nostim_on_fin_temps);
    nostim_on_fin_SEMs(k) = std(nostim_on_fin_temps)/sqrt(length(nostim_on_fin_temps));
...
figure();
hold on;
errorbar(param_array, stim_off_fin_avgs, stim_off_fin_SEMs, "ro", "DisplayName","stayed off with stim");
errorbar(param_array, nostim_on_fin_avgs, nostim_on_fin_SEMs, "go", "DisplayName","turned on without stim");
title(strcat("Effect of ",param_name," on Unit Activity Switching"))
