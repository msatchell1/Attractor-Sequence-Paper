...
LCO1 = LinClassObj; % Intialize linear classification object.

LCO1 = param_sweep_genr(LCO1); % Run parameter sweep over simulations.
...
% typecomb2 data] , ... ]
X = reshape(LCO1.r_avg, [(size(LCO1.r_avg,1)*size(LCO1.r_avg,2)), size(LCO1.r_avg,3)]);
X_types = reshape(LCO1.types, [(size(LCO1.types,1)*size(LCO1.types,2)), size(LCO1.types,3)]);

% Assigns this data to the linear classification parameters structure:
LCO1.X = X;
LCO1.X_types = X_types;
...
for char = 1:size(X_types, 2)
   if char == LCO1.clsf_index(1)
       % Finds the index locations in the column of X_types that have
       % correct type labels.
      indices = find(X_types(:,char) == LCO1.clsf_index(2));
...
% data.
[LCO1.Mdl, LCO1.FitInfo] = fitclinear(LCO1.X, LCO1.X_labels);
