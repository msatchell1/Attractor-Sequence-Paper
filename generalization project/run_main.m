...
p = make_params_genr('rng_seed',3, 'num_type',2, 'num_char',2);
...
all_r_avg = zeros(size(type_combs,2),p.Ne); % Array to hold final avg firing rates for
% all possible stimuli. For excitatory units.

for j = 1:size(type_combs,2) % Loops through all possible type input combinations.

    stim_choice = num2cell(type_combs(:,j)); % Selects a stimulus and turn it 
